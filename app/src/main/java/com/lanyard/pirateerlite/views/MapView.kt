...
import com.lanyard.library.CardinalSpline
...
class MapView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        ...
        val gson = Gson()
        val reader = JsonReader(InputStreamReader(context.assets.open("map_view.json")))
    ...
    fun makeAnimations(context: Context, data: HashMap<String, Any>) {
        val anim = data["Animations"] as ArrayList<ArrayList<Any>>
        for (atom in anim) {
            val name = atom[0] as String
            val framenames = data[name] as ArrayList<String>
            val bimpl = ArrayList<BitmapStream>()
    ...
    fun makeFlags(context: Context, data: HashMap<String, Any>) {
        ...
        val framenames = data["flags"] as ArrayList<String>
    ...
    fun plotJobMarkers(boat: BoatModel) {
        _jobNode.removeAllChildren()
        val towns = mutableSetOf<TownModel>()
        boat.cargo.forEach {
            if (it != null) {
    ...
    fun plotRoutesForTown(town: TownModel, paths: MutableList<List<Edge<WorldNode>>>, distance: Float) {
        ...
        val stroke = context.resources.getDimensionPixelSize(R.dimen.boat_range_stroke_size).toFloat()
    ...
    fun showCourseTrail(path: List<Edge<WorldNode>>, image: String) {
        val spline = CardinalSpline(Graph.getRoutePositions(path))
    ...
    fun plotCourseForBoat(boat: BoatController) {
        ...
        val rem = arrayListOf<CanvasNode>()
        for (i in 0 until size) {
            val rtime = size * percent
            if (i <= rtime.toInt()) {
                rem.add(_plotNode.children[i])
            } else {
    ...
    fun showMoney(town: TownModel, gold: Int, silver: Int) {
        ...
        for (i in 0 until arr.size step 3) {
            ...
            label.position.set(
                image.position.x + (image.size.width.toInt() * 0.5f).toInt() + (label.size.width * 0.5f).toInt(),
                image.position.y
            )
        ...
        val messageSize = Size(messageWidth, messageHeight)
